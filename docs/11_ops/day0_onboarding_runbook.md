...
- [ ] Approve each workspace root and run `forge onboard`; review the
  discovered repository list before deeper inspection.
- [ ] Review the practical-full plan, exact digest, user-level actions,
  privileged scripts, agent blocks, and optional PATH/configuration change.
- [ ] Execute only approved user-level actions. Run privileged scripts
  personally after review, then rerun readiness.
...
- [ ] Never let Forge invoke elevation. Keep credentials and global Git
  configuration out of the plan.
