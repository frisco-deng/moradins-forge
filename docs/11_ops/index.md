---
title: "11 Ops Index"
status: approved
owner: platform-architecture
last_reviewed: 2026-07-31
source_refs:
  - https://openai.com/index/harness-engineering/
  - https://openai.com/index/unlocking-the-codex-harness/
  - https://medium.com/@DataDo/building-production-grade-agentic-rag-a-technical-deep-dive-part-1-beyond-fixed-windows-9879cf3cc7b1
related_docs:
  - ../00_overview/engineer_entrypoint.md
  - docs/exec_plans/index.md
---

# 11 Ops Index

## Purpose

- Provide deterministic navigation for this section and define the expected read order.

## Section Map

- [Agent Harness Governance](agent_harness_governance.md)
- [Codex Run Loop](codex_run_loop.md)
- [Tooling Pipeline](tooling_pipeline.md)
- [Manager Domain Lanes And Local Worker Follow-On](domain_lane_follow_on.md)
- [Documentation Review Loop](documentation_review_loop.md)
- [Engineer Entry Authoring Runbook](engineer_entry_authoring_runbook.md)
- [Quick Start](quick_start.md)
- [Air-Gapped Tooling Suite](air_gapped_tooling_suite.md)
- [Day 0 Onboarding Runbook](day0_onboarding_runbook.md)
- [Day 1 Onboarding Runbook](day1_onboarding_runbook.md)
- [Change Tracking System](change_tracking_system.md)
- [Archive Process](archive_process.md)
- [Git Workflow Gitlab](git_workflow_gitlab.md)
- [Deployment Environments](deployment_environments.md)
- [Docker Compose](docker_compose.md)
- [Local Dev](local_dev.md)
- [Project Builder Runbook](project_builder_runbook.md)
- [Project Builder SSH Operator Guide](project_builder_ssh_operator_guide.md)
- [Configuration](configuration.md)
- [Capacity Planning](capacity_planning.md)
- [Reprocessing And Backfills](reprocessing_and_backfills.md)
- [Upgrades](upgrades.md)
- [Disaster Recovery](disaster_recovery.md)

## Read Order

1. [Agent Harness Governance](agent_harness_governance.md)
2. [Codex Run Loop](codex_run_loop.md)
3. [Tooling Pipeline](tooling_pipeline.md)
4. [Manager Domain Lanes And Local Worker Follow-On](domain_lane_follow_on.md)
5. [Documentation Review Loop](documentation_review_loop.md)
6. [Quick Start](quick_start.md)
7. [Air-Gapped Tooling Suite](air_gapped_tooling_suite.md)
8. [Engineer Entry Authoring Runbook](engineer_entry_authoring_runbook.md)
9. [Day 0 Onboarding Runbook](day0_onboarding_runbook.md)
10. [Day 1 Onboarding Runbook](day1_onboarding_runbook.md)
11. [Change Tracking System](change_tracking_system.md)
12. [Archive Process](archive_process.md)
13. [Git Workflow Gitlab](git_workflow_gitlab.md)
14. [Deployment Environments](deployment_environments.md)
15. [Docker Compose](docker_compose.md)
16. [Local Dev](local_dev.md)
17. [Project Builder Runbook](project_builder_runbook.md)
18. [Project Builder SSH Operator Guide](project_builder_ssh_operator_guide.md)
19. [Configuration](configuration.md)
20. [Capacity Planning](capacity_planning.md)
21. [Reprocessing And Backfills](reprocessing_and_backfills.md)
22. [Upgrades](upgrades.md)
23. [Disaster Recovery](disaster_recovery.md)

## Operating Notes

- Start at `docs/00_overview/engineer_entrypoint.md` for active goals and constraints.
- Treat this section as complete only when every child document has explicit decisions and no placeholder language.

## Verification Checklist

- [ ] Every sibling doc is linked in Section Map.
- [ ] Read order matches intended onboarding path for this section.
- [ ] Related docs include upstream and operational anchors.

## Related Docs

- ../00_overview/engineer_entrypoint.md
- docs/exec_plans/index.md
