...
- If deeper control-plane docs conflict with this file, follow the deeper docs.
- Keep this root file compact; detailed policy belongs in docs and contracts.
...
- Compatibility aliases remain only as temporary bridges during the migration
  window.
...
- Never commit secrets, tokens, local credentials, or private host paths.
...
- Use descriptive commit messages and ISO 8601 dates (`YYYY-MM-DD`) when dates
  matter.
- Do not merge or promote release branches without the required human gate.
