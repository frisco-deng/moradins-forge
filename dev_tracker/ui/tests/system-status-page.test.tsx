...
const mockSnapshot = {
  ...
  qa_signals: {
    ...
    branch_hygiene: { status: "fail", detail: {} },
    documentation_review: { status: "warn", detail: {} },
  },
  git: {
    version: "GitStateV1",
    branch: "harness/system-status",
    short_sha: "abc123",
    last_commit: "abc123 | 2026-03-03 | system status",
    dirty: false,
    markdown_changed_count: 0,
...
const mockStatus = {
  ...
  remote_ssh: {
    ...
    allowed_command_prefixes: ["pwd", "ls", "whoami", "uname -a"],
    mode: "guarded",
  },
  builder_feature_flags: {
    managed_by: "environment",
    existing_project_mode_enabled: false,
...
describe("system status page", () => {
        ...
        sshProfiles: [
          {
            id: "ssh-profile-1",
            label: "Remote Status",
            target_id: "remote-status",
            connection_mode: "ssh",
            host: "remote.example",
            user: "ops",
            port: 22,
            allowlisted_root: "/srv/work",
            profile_label: "remote-status",
        ...
        if (url.includes("/api/builder/remote/ssh/execute")) {
          return new Response(
            JSON.stringify({
              ...
              status: "pass",
              command: "ls -- 'remote-existing-project/.moradins-harness'",
              stdout: "AGENTS.md\nREADME.md\n",
              stderr: "",
              exit_code: 0,
            }),
  ...
  it("renders runtime and guardrail diagnostics", async () => {
    render(
      <TrackerProvider>
    ...
    await waitFor(() => {
      expect(screen.getByRole("heading", { name: "Control API Runtime" })).toBeInTheDocument();
    });

    expect(screen.getByText("Control API Runtime")).toBeInTheDocument();
    expect(screen.getByText("Builder Flags")).toBeInTheDocument();
    expect(screen.getByText("Guardrails")).toBeInTheDocument();
    expect(screen.getByText("SSH Profiles and Connection Checks")).toBeInTheDocument();
    expect(screen.getByText("UI Access and Companion Runtime")).toBeInTheDocument();
    expect(screen.getByText("Allowlisted Commands")).toBeInTheDocument();
    expect(screen.getByText("Assistant Actions")).toBeInTheDocument();
  });

  it("builds repo-aware remote sidecar checks from the selected profile", async () => {
    ...
    await user.type(await screen.findByLabelText("Target Repo"), "remote-existing-project");
    await user.click(screen.getAllByRole("button", { name: "Check Sidecar" })[0]);
