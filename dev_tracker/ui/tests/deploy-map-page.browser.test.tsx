...
test("renders deploy map workflow and cached builder output in browser mode", async () => {
  ...
  await expect.element(screen.getByRole("button", { name: /Current Project Sidecar/i })).toBeVisible();
  await expect.element(screen.getByRole("navigation", { name: "Deploy Workspace navigation" }).getByRole("link", { name: "Builder" })).toBeVisible();
  await expect.element(screen.getByRole("navigation", { name: "Deploy Workspace navigation" }).getByRole("link", { name: "Verify" })).toBeVisible();
  await expect.element(screen.getByText("Last builder output loaded")).toBeVisible();
  await expect.element(screen.getByRole("heading", { name: "Baseline Harness Tree" })).toBeVisible();
});
