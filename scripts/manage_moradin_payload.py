...
def validate_template_root(template_root: Path) -> ValidationResult:
    ...
    for forbidden in FORBIDDEN_TEMPLATE_PATHS:
        if (template_root / forbidden).exists():
            messages.append(f"forbidden manager path leaked into template: {forbidden}")
    for forbidden in FORBIDDEN_TEMPLATE_FILES:
        if (template_root / forbidden).exists():
            messages.append(
                f"forbidden manager-only file leaked into template: {forbidden}"
            )
...
def validate_manager_repo(repo_root: Path) -> ValidationResult:
    ...
    for required_file in required_files:
        if not (repo_root / required_file).exists():
            label = "public Forge" if public_mode else "manager"
...
def validate_materialized_target(target_root: Path) -> ValidationResult:
    ...
    for forbidden in FORBIDDEN_TEMPLATE_PATHS:
        if (target_root / forbidden).exists():
            messages.append(
                f"materialized target leaked manager-only path: {forbidden}"
            )
    for forbidden in FORBIDDEN_TEMPLATE_FILES:
        if (target_root / forbidden).exists():
            messages.append(
                f"materialized target leaked manager-only file: {forbidden}"
            )
    for legacy_path in LEGACY_ALIAS_PATHS:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate and dry-run the Moradin Harness payload."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    validate_parser = subparsers.add_parser(
        "validate",
        help="Validate the manager repo, Moradin payload manifest, and compatibility scaffold.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print_result("manager", manager_result)
